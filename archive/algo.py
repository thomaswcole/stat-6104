...
def generate_sparse_matrix(m, r, delta=0.01):
    """Correct sparse matrix generation A = X_GT Y_GT + N"""
    X_GT, Y_GT, n = generate_ground_truth(m, r, delta)
    N = generate_noise_matrix(m, n, delta)
    
    # Efficient sparse matrix product
    A = np.zeros((m, n))
    
    # Find non-zero entries in X_GT and Y_GT
    rows, cols = np.nonzero(X_GT)
    y_rows, y_cols = np.nonzero(Y_GT)
    
    # Create dictionary for faster column access in Y
    y_dict = {}
    for r, c in zip(y_rows, y_cols):
        if r not in y_dict:
            y_dict[r] = []
        y_dict[r].append(c)
    
    # Compute product only for non-zero combinations
    for i, k in zip(rows, cols):
        if k in y_dict:  # If column k of X has non-zero and row k of Y has non-zero
            for j in y_dict[k]:
                A[i,j] += X_GT[i,k] * Y_GT[k,j]
